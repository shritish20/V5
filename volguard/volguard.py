...
class DatabaseWriter:
    ...
    def _worker(self):
        ...
        while self.running:
            try:
                priority, _, msg = self.message_queue.get(timeout=1)
                if msg['type'] == 'execute':
    ...
    def execute(self, sql: str, params: tuple = (), priority: int = None, timeout: float = 5.0):
        ...
        try:
            self.message_queue.put((priority, time.time_ns(), message), timeout=timeout)
        except queue.Full:
            if priority <= self.PRIORITY_HIGH:
                ...
                while not self.message_queue.empty():
                    try:
                        p, _, m = self.message_queue.get_nowait()
                        if p >= self.PRIORITY_NORMAL:
                            dropped += 1
                        else:
                            temp_queue.put((p, _, m))
                    except:
                        break
                ...
                try:
                    self.message_queue.put((priority, time.time_ns(), message), timeout=1.0)
    ...
    def executescript(self, sql: str, timeout: float = 5.0):
        try:
            self.message_queue.put((self.PRIORITY_NORMAL, time.time_ns(), {'type': 'executescript', 'sql': sql}), timeout=timeout)
        except queue.Full:
    ...
    def shutdown(self):
        ...
        self.running = False
        try:
            self.message_queue.put((0, time.time_ns(), {'type': 'shutdown'}), timeout=2)
        except queue.Full:
            pass
